...
def infinite_well_plot(E,V,xvec,W,steps,n,Case,ask_to_save=False,ask_squared=False):
    ...
    plt.show()
def finite_well_plot(E,V,xvec,steps,n,Case,U,param,ask_to_save=False,ask_squared=False):
    if ask_squared:
        V = np.multiply(np.conj(V),V)
    ...
    for i in range(np.size(param)):
        title+=param[i].description+"="+str(param[i].value)+", "
    ax.set_title(title)
    # set x label
    plt.xlabel('Width of Well / (a.u.)')
    # set y label
